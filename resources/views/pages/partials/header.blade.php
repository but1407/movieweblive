                    ...
                    <ul id="menu-menu_1" class="nav navbar-nav navbar-left">
                            ...
                            <ul role="menu" class=" dropdown-menu">
                                @foreach ($genre as $gen)
                                    <li><a title="{{ $gen->title }}"
                                            href="{{ route('genre', ['slug' => $gen->slug]) }}">{{ $gen->title }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </li>
                            ...
                            <ul role="menu" class=" dropdown-menu">
                                @foreach ($country as $key => $cou)
                                    <li><a title="{{ $cou->title }}"
                                            href="{{ route('country', ['slug' => $cou->slug]) }}">{{ $cou->title }}</a>
                                    </li>
                                @endforeach
                        ...
                        @foreach ($category as $key => $cate)
                            <li class="mega"><a title="{{ $cate->title }}"
                                    href="{{ route('category', ['slug' => $cate->slug]) }}">{{ $cate->title }}</a></li>
                        @endforeach
                        <li class="mega dropdown">
                            ...
                            <ul role="menu" class="dropdown-menu">
                                @for ($year = 1990; $year <= Carbon\Carbon::now()->year; $year++)
                                    <li><a title="Phim 2020" href="{{ route('year', $year) }}">Phim
                                            {{ $year }}</a></li>
                                @endfor
                            </ul>
                        </li>
